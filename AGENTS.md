...
> Repository development instructions for **analytics** (Service).
...
src/adapter, analytics-engine, api-key, catalog, config, domain-ingestion, feature-flags, graphql, health, ingestion, lineage, processing, replay, scheduler, search
...
@omnixys/contracts-ts, cache-ts, context-ts, graphql-ts, grpc-ts, http-ts, kafka-ts, logger-ts, observability-ts, security-ts
...
node --test test/*.test.mjs (unit, error-contract); test:helm validates deploy/helm/analytics-service

## Repository-Specific Rules

Helm chart under deploy/helm/analytics-service. Multi-tenant analytics data (tenant predicates mandatory).
