# 🧾 Changelog

All notable changes in this project will be documented in this file.


## [2.0.4](https://github.com/omnixys/analytics-service/compare/v2.0.3...v2.0.4) (2026-09-13)

### Deps

* **Deps:** update omnixys packages ([](https://github.com/omnixys/analytics-service/commit/169ee502056fba3346e923f5fed1d58a680d5f10))

## [2.0.3](https://github.com/omnixys/analytics-service/compare/v2.0.2...v2.0.3) (2026-09-12)

### Analytics

* **Analytics:** bump omnixys packages to 4.4.0 ([](https://github.com/omnixys/analytics-service/commit/df44ac200575c2a2ee10bb1b2e5e781f943449e0))

## [2.0.2](https://github.com/omnixys/analytics-service/compare/v2.0.1...v2.0.2) (2026-09-11)

### Deps

* **Deps:** update omnixys contracts-ts package ([](https://github.com/omnixys/analytics-service/commit/15b45e43453a4d608811cf32060bb19a8e26cc8d))

## [2.0.1](https://github.com/omnixys/analytics-service/compare/v2.0.0...v2.0.1) (2026-09-10)

### Deps

* **Deps:** update graphql-ts ([](https://github.com/omnixys/analytics-service/commit/b0952bf18832daca3177b96419960c72b26b9f04))

## [2.0.0](https://github.com/omnixys/analytics-service/compare/v1.2.5...v2.0.0) (2026-09-05)

### Analytics

* **Analytics:** rewire omnixys service deps to file: ([](https://github.com/omnixys/analytics-service/commit/12989a0081a6f7fb584edc11752f682ddae8343d))
* **Analytics:** type identity reference fields as UUID ([](https://github.com/omnixys/analytics-service/commit/ff5aad005ac18786cffc185c3483a74a717cb5f4))
* **Analytics:** accept UUIDv7 in id patterns ([](https://github.com/omnixys/analytics-service/commit/1a9f9533e6b039bf6ccf801c4f41781e7500ed05))

### Deps

* **Deps:** update omnixys deps ([](https://github.com/omnixys/analytics-service/commit/851920ecd4f7732200a337a1741cdddabdb56fc6))

### Identity

* **Identity:** document U/K identity conventions in AGENTS.md ([](https://github.com/omnixys/analytics-service/commit/2bc5460174ae46066a1985317dd8c29b72e4e5fc))

### Logging

* **Logging:** add service:analytics provenance source to logger acquisitions ([](https://github.com/omnixys/analytics-service/commit/21673e9ecc14bf0b2389d811e88e0c39c436c060))
* **Logging:** add missing format placeholders to structured log calls ([](https://github.com/omnixys/analytics-service/commit/b9e0c1aa8535d40d37d0ca589c4bb161789db615))

### Other

* **Other:** Merge pull request #1 from omnixys/migration/uuid-v7 ([](https://github.com/omnixys/analytics-service/commit/b8e27de026da97fa86d4ca0a553291b06c1e549f)), closes [#1](https://github.com/omnixys/analytics-service/issues/1)

### Runtime

* **Runtime:** require node 26.8.1 and pnpm 11.24.0 ([](https://github.com/omnixys/analytics-service/commit/1bf215305e5ef35719704c12e609d95514c90129))

## [1.2.5](https://github.com/omnixys/analytics-service/compare/v1.2.4...v1.2.5) (2026-08-28)

### Deps

* **Deps:** update contracts-ts ([](https://github.com/omnixys/analytics-service/commit/1ae0cba8485fa7657a691ffe4f0c94135928eda8))

## [1.2.4](https://github.com/omnixys/analytics-service/compare/v1.2.3...v1.2.4) (2026-08-26)

### Build

* **Build:** fix build errors ([](https://github.com/omnixys/analytics-service/commit/0ab70b9529cc91cd090b6de1bffc9f38310df5ec))

## [1.2.3](https://github.com/omnixys/analytics-service/compare/v1.2.2...v1.2.3) (2026-08-26)

### Build

* **Build:** fix build errors ([](https://github.com/omnixys/analytics-service/commit/23c2421c1262ede0c9705f197edf5ad424a002d2))

### Other

* **Other:** Merge branch 'main' of https://github.com/omnixys/analytics-service ([](https://github.com/omnixys/analytics-service/commit/a7d46b2b27e09afe7431d5a03b760aab1134aaec))

## [1.2.2](https://github.com/omnixys/analytics-service/compare/v1.2.1...v1.2.2) (2026-08-26)

### Build

* **Build:** fix build errors ([](https://github.com/omnixys/analytics-service/commit/ecd23d59dcd68a5f392c74b0d541f73a2ebbdb65))

## [1.2.1](https://github.com/omnixys/analytics-service/compare/v1.2.0...v1.2.1) (2026-08-26)

### Deps

* **Deps:** update logger to 3.2.6 ([](https://github.com/omnixys/analytics-service/commit/bc89ad439bab82b70c6aaa5acf74d50f34cf1318))
* **Deps:** update shared TypeScript packages ([](https://github.com/omnixys/analytics-service/commit/0e5260773020c11d5e6a0638f9ed1d8e18c36f64))

## [1.2.0](https://github.com/omnixys/analytics-service/compare/v1.1.3...v1.2.0) (2026-08-26)

### Deps

* **Deps:** update omnixys ts packages ([](https://github.com/omnixys/analytics-service/commit/135c3bb3451a09cfdd2a5fb00ff16e1fd527cbdd))

### Otel

* **Otel:** add otel logs ([](https://github.com/omnixys/analytics-service/commit/74fff5f3c7136faca79093819f97e4cfb7a169b3))

## [1.1.3](https://github.com/omnixys/analytics-service/compare/v1.1.2...v1.1.3) (2026-08-23)

### Logger

* **Logger:** fix terminal logger ([](https://github.com/omnixys/analytics-service/commit/89db9a248a178c21f886b56119b659592faa207a))

## [1.1.2](https://github.com/omnixys/analytics-service/compare/v1.1.1...v1.1.2) (2026-08-23)

### Observability

* **Observability:** update dependency ([](https://github.com/omnixys/analytics-service/commit/ac8b452a0959d172f051596daa5a2cc24bdd5bef))

## [1.1.1](https://github.com/omnixys/analytics-service/compare/v1.1.0...v1.1.1) (2026-08-19)

### Agent

* **Agent:** add repository development instructions ([](https://github.com/omnixys/analytics-service/commit/67f6f4d2251de6ab653f8ec1104dce46161dfe3a))

### Analytics

* **Analytics:** exclude health endpoints from rate-limit and bump version ([](https://github.com/omnixys/analytics-service/commit/6c8a3140947e3bfbc481663d3be6c5265f6ca838))
* **Analytics:** use allowList for rate-limit skip and fix pnpm overrides ([](https://github.com/omnixys/analytics-service/commit/b73813c8383d01d5319c2a00db38be0ce1509a7c))

## [1.1.0](https://github.com/omnixys/analytics-service/compare/v1.0.0...v1.1.0) (2026-08-04)

### Wedding

* **Wedding:** add more analytics metrics ([](https://github.com/omnixys/analytics-service/commit/21e5e762e74e5fb98369eadaad768a3dbafc22dc))

## 1.0.0 (2026-08-03)

### Analytics

* **Analytics:** add checkpoint demo seed and validation ([](https://github.com/omnixys/analytics-service/commit/a9101118b59c3ec5ba083e8ae6f92feaac102b94))
* **Analytics:** gate ingestion rollouts ([](https://github.com/omnixys/analytics-service/commit/363a7754531c3847ff2deeda630e85172f775f34))
* **Analytics:** implement event processing pipeline ([](https://github.com/omnixys/analytics-service/commit/ba0b26ab59ca82b276e207fc8c3beec44e22c2ed))
* **Analytics:** ingest approved domain facts ([](https://github.com/omnixys/analytics-service/commit/fee72c51978c3ef7c815c8062575d75b6d8cea8b))
* **Analytics:** resolve build, lint, and runtime errors ([](https://github.com/omnixys/analytics-service/commit/d9bb28f47b507a302cfa0ed153c2e96443c7ad14))
* **Analytics:** tenant integration with -ts packages and Prisma 7 client ([](https://github.com/omnixys/analytics-service/commit/969a034952e5db9a6d4f9f58e6606d1a6f9fd359))

### Architecture

* **Architecture:** record analytics platform decisions ([](https://github.com/omnixys/analytics-service/commit/a6f4f09c81bf48da2b2b5e97b5aae4f432051a33))

### Auth

* **Auth:** issue browser analytics tokens ([](https://github.com/omnixys/analytics-service/commit/0700455c861fb3bd368379bb013b35e12b3ffd37))

### Banner

* **Banner:** update banner ([](https://github.com/omnixys/analytics-service/commit/ccbcdf981caa01c9e6cc736a48b1415379b59b1f))

### Deploy

* **Deploy:** add production readiness assets ([](https://github.com/omnixys/analytics-service/commit/bd7274a56ac141aa19294529f8f3736a68aa4218))

### Errors

* **Errors:** adopt secure analytics error handling ([](https://github.com/omnixys/analytics-service/commit/3f26b83b493bb4aea8107ef895b7835f382da3be))

### Feature-flags

* **Feature-flags:** evaluate rollouts and track exposures ([](https://github.com/omnixys/analytics-service/commit/304012f24540dfe367c7e7665ceadf2914fc4882))

### Health

* **Health:** expose dependency readiness ([](https://github.com/omnixys/analytics-service/commit/4734db62791e6b19a0410f52609a39ce5abc44fd))

### Ingestion

* **Ingestion:** add validated batch pipeline ([](https://github.com/omnixys/analytics-service/commit/52351374ce8fb452819e8cae22610a5d7a0de624))

### Init

* **Init:** initial commit ([](https://github.com/omnixys/analytics-service/commit/13ebbe69de87ec5a7ed592e69249d1593381ca50))

### Lineage

* **Lineage:** add metric dependency graph ([](https://github.com/omnixys/analytics-service/commit/9230bc01391065941e8932ba7ff4927c725f6a15))

### Metrics

* **Metrics:** add incremental semantic analytics engine ([](https://github.com/omnixys/analytics-service/commit/e7aeb91f8e7324a61d94cf3b915ce48bf58f297f))

### Operations

* **Operations:** record production release gates ([](https://github.com/omnixys/analytics-service/commit/1afacb334d0ae745d10598eaa2b4f7e256b19197))

### Platform

* **Platform:** add analytics service foundation ([](https://github.com/omnixys/analytics-service/commit/efe5cc413396296a22eae291ab161da8652f6067))

### Quality

* **Quality:** enforce service lint and type checks ([](https://github.com/omnixys/analytics-service/commit/9a72ca98c7ca90994affdc36a0b099e535f2f226))

### Replay

* **Replay:** add tenant replay queue ([](https://github.com/omnixys/analytics-service/commit/301e5ddb8803dce0bf1812f0add9e3b82a7d7d8e))

### Rules

* **Rules:** execute safe event actions ([](https://github.com/omnixys/analytics-service/commit/d78c086efe29d39490b85685dfed8b5834ba35dc))

### Scheduler

* **Scheduler:** run persistent timezone aware jobs ([](https://github.com/omnixys/analytics-service/commit/c05d0699631665d2fd24834897a51fb6ecdc28ac))

### Search

* **Search:** add tenant analytics search ([](https://github.com/omnixys/analytics-service/commit/ca2ba2bf5ec333aa68e69eeb4841d7e803701679))

### Semver

* **Semver:** add semver ([](https://github.com/omnixys/analytics-service/commit/1dd9565387eba8f4c57b2cbcf64ab55c089eee79))

### Update

* **Update:** Update package.json ([](https://github.com/omnixys/analytics-service/commit/c3db005d363fc7becca9c381d54c29a5043a782d))
